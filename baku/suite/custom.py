...
class DummyTimeStep:
    def __init__(self, observation: dict, reward: float = 0.0, done: bool = False):
...
class DummyEnv:
    def __init__(
        ...
        **kwargs,
        ...
        self._action_dim = int(action_dim) if action_dim is not None else int(max_action_dim or 0)
        self._max_episode_len = int(max_episode_len)
        self.current_step = 0

    def observation_spec(self):
        # return objects exposing .shape (numpy arrays are fine)
    ...
    def reset(self) -> DummyTimeStep:
        self.current_step = 0
        obs = {"features": np.zeros(self._state_dim, dtype=np.float32)}
        return DummyTimeStep(observation=obs, reward=0.0, done=False)

    def step(self, action) -> DummyTimeStep:
        self.current_step += 1
        done = self.current_step >= self._max_episode_len
        obs = {"features": np.zeros(self._state_dim, dtype=np.float32), "goal_achieved": 0}
        return DummyTimeStep(observation=obs, reward=0.0, done=done)
...
def task_make_fn(
    ...
    **env_kwargs,
    ...
    # accept either a config node or an instantiated dataset
    if isinstance(dataset, (dict, DictConfig)):
        dataset = hydra.utils.call(dataset)

    # don't forward config-only keys to env ctor
    env_kwargs = dict(env_kwargs)
    env_kwargs.pop("max_action_dim", None)
...
class CustomSuite:
    ...
    history: bool = False
    history_len: int = 1
    eval_history_len: int = 1
    task_make_fn: Any = task_make_fn