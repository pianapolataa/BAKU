...
class CustomTeleopBCDataset(IterableDataset):
    ...
    def _sample(self):
        idx = np.random.randint(0, self._num_samples)
        features = self.normalized_features[idx]          # (feature_dim,)
        actions = self.normalized_actions[idx]           # (action_dim,)
        pixels = self.observations[idx]["pixels0"].astype(np.float32)  # (3, H, W)

        # --- Tile features across history_len ---
        feat = np.tile(features[None, :], (self.history_len, 1)).astype(np.float32)  # (T, feature_dim)

        # --- Tile task embedding across history_len ---
        task_emb_t = np.tile(self.task_emb[None, :], (self.history_len, 1)).astype(np.float32)  # (T, E)

        # --- Tile pixels across history_len ---
        pixels_t = np.tile(pixels[None, :, :, :], (self.history_len, 1, 1, 1)).astype(np.float32)  # (T, 3, H, W)

        # --- Tile actions ---
        if self.temporal_agg:
            sampled_actions = np.tile(actions.reshape(1, 1, -1),
                                    (self.history_len, self.action_repeat, 1)).astype(np.float32)  # (T, action_repeat, action_dim)
        else:
            sampled_actions = np.tile(actions.reshape(1, 1, -1),
                                    (self.history_len, 1, 1)).astype(np.float32)  # (T, 1, action_dim)

        return {
            "pixels0": pixels_t,       # (T, 3, H, W)
            "features": feat,          # (T, feature_dim)
            "actions": sampled_actions, # (T, action_repeat, action_dim) or (T, 1, action_dim)
            "task_emb": task_emb_t,    # (T, E)
        }
